...
func createPair{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (token0, token1) = StarkExLibrary.sortTokens(tokenA, tokenB)

    # Pair exists
    with_attr error_message("StarkEx: PE"):
    ...
    IStarkExPair.initialize(contract_address=newPair, token0=token0, token1=token1)
